...
program
    ...
    .option('--cloud', 'Create a project with an actions server to be used with Clai Cloud.')
    ...
    .description('Create a new Clai project.')
    .action(initCommand);
...
program
    ...
    .option('--ci', 'No spinners, no prompt confirmations')
    .description('Start a Clai project.  Must be executed in your project\'s directory')
    .action(dockerComposeUp);

program
    .command('down')
    .option('-v, --verbose', 'Display Docker Compose start-up logs')
    .description('Stops a Clai project and releases Docker resources.  Must be executed in your project\'s directory')
...
program
    .command('killall')
    .option('--remove-images', 'Will also remove Clai related Docker images')
    .description('Stops any running Clai project')
...
program
    ...
    .description('Stop a Clai service (interactive). Must be executed in your project\'s directory')
    .action(dockerComposeStop);

program
    .command('start <service>')
    .description('Start a Clai service (interactive). Must be executed in your project\'s directory')
    .action(dockerComposeStart);

program
    .command('restart <service>')
    .description('Restart a Clai service (interactive). Must be executed in your project\'s directory')
...
program
    .command('update')
    .description('Update a project if the current Clai version as a higher minor version but not a different major version')
...
async function killAllCommand(cmd) {
    const { stop } = await inquirer.prompt({
        type: 'confirm',
        name: 'stop',
        message: 'This will stop any running Clai project and cleanup remaining Docker resources. This will not affect your project\'s data. Proceed ?',
        default: true,
    });
    if (stop){
        const spinner = ora();
    ...
    try {
        ...
        if (isProjectDir()){
            if (containers && containers.length){
                choices.push({ title: 'Stop Clai', cmd: () => dockerComposeDown({ verbose: false }) });
            ...
            if (isMinorUpdate()){
                choices.push({ title: `Update project to Clai ${getBotfrontVersion()}`, cmd: doMinorUpdate});
            }
        } else {
            if (containers && containers.length){
                choices.push({ title: 'Stop Clai', cmd: () => killAllCommand({ verbose: false }) });
            }
        ...
        console.log(boxen(`Welcome to ${chalk.green.bold('Clai')}!\nversion: ${getBotfrontVersion()}`,  { padding: 1,  margin: 1 }));
