...
export async function verifySystem() {
    ...
    if (!result.object)
        throw `You must install Docker to use Clai. Please visit ${chalk.green(
            'https://www.docker.com/products/docker-desktop',
        )}`;
    const results = await promisify(check)({ node: '>= 8.9' });
    if (!results.versions.node.isSatisfied) {
        throw `You must upgrade your Node.js installation to use Clai. Please visit ${chalk.green(
