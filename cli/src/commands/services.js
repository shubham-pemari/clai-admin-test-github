...
async function postUpLaunch(spinner) {
    ...
    setSpinnerText(
        spinner,
        `Opening Clai (${chalk.green.bold(serviceUrl)}) in your browser...`,
    );
    await wait(2000);
...
export async function doMinorUpdate() {
    ...
    if (isMajorUpdateWithVersion(projectVersion, botfrontVersion)) {
        return console.log(
            boxen(
                `Project was made with Clai ${chalk.blueBright(
                    projectVersion,
...
export async function dockerComposeUp(
    ...
    startSpinner(spinner, 'Starting Clai...');
    ...
    try {
        ...
        stopSpinner();
        console.log(`\n\n        🎉 🎈  Clai is ${chalk.green.bold('UP')}! 🎉 🎈\n`);
        const message =
            'Useful commands:\n\n' +
            (`\u2022 Run ${chalk.cyan.bold('clai logs')} to follow logs \n` +
                `\u2022 Run ${chalk.cyan.bold(
                    'clai watch',
                ...
                `\u2022 Run ${chalk.cyan.bold('botfront down')} to stop Clai\n` +
                `\u2022 Run ${chalk.cyan.bold(
                    'botfront --help',
        ...
        if (verbose) {
            ...
            stopSpinner(spinner);
            failSpinner(spinner, 'Couldn\'t start Clai. Retrying in verbose mode...', {
                exit: false,
            });
...
export async function dockerComposeDown({ verbose }, workingDir) {
    ...
    if (!isProjectDir()) {
        ...
        const noProjectMessage =
            ...
            `${killall} will find and shut down any Clai\nproject on your machine.`;
        return console.log(boxen(noProjectMessage));
    }
    const spinner = ora('Stopping Clai...');
    spinner.start();
