            ...
            .then(activities => activities.forEach(async (activity) => {
                if (typeof activity._id !== 'string') {
                    try {
                        ...
                        console.log(
                            'something went wrong during a migration, contact Clai for further help',
                        );
                    }
                }
            }));
...
Migrations.add({
    ...
    up: async () => {
        ...
        storyGroups.forEach(sg => StoryGroups.update(
            ...
            {
                $set: {
                    //isExpanded: !!sg.introStory,  -->removed to fix concurrency list view issue
    ...
    up: async () => {
        ...
        dashboards.forEach(async (dashboard) => {
            ...
            const newCards = cards.map((card) => {
                ...
                if (card.type === 'sentimentAnalysis') {
                    return { ...card, limit: '10' };
                }
                return card;
            });
...
//migration 27 added to include a new attribute "expandedUsers" in the database (A.C)
Migrations.add({
    version: 27,
    up: async () => {
        const { webhooks } = safeLoad(
...
Migrations.add({
    ...
    up: async () => {
        try {
            const currentSlots = Slots.find().fetch();
            console.log("current slots schema",currentSlots)
            currentSlots.forEach((slot) => {
                if(slot.type == "unfeaturized") {
                    Slots.update({ _id: slot._id },{ $set: { type: "text" } },)
                }
            })
        } catch (e) {
            throw new Error(
                `Something went wrong during a migration, contact Clai for further help: ${e}`,
            );
        }
    }
})
