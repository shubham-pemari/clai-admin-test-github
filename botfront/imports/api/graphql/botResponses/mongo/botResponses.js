...
import {
    getWebhooks, deleteImages, deleteVideos, auditLogIfOnServer, getImageUrls, getVideoUrls
} from '../../../../lib/utils';
...
export const deleteResponse = async (projectId, key) => {
    ...
    // const { deleteVideoWebhook: { url, method } } = getWebhooks();
    if (url && method) deleteImages(getImageUrls(response), projectId, url, method);
    // if (url && method) deleteVideos(getVideoUrls(response), projectId, url, method);
