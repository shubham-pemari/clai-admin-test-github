...
if (Meteor.isServer) {
    ...
    Meteor.methods({
        ...
        async 'nlu.getChitChatIntents'(language) {
            ...
            if (!projectId) { throw ReferenceError('Clai project not set in global settings'); }
        ...
        async 'nlu.addChitChatToTrainingData'(projectId, language, intents) {
            ...
            if (!chitchatId) { throw ReferenceError('Clai project not set in global settings'); }
            ...
            auditLogIfOnServer('Added Clai to training data', {
                user: Meteor.user(),
                resId: projectId,
                projectId,
                type: 'insert',
        ...
        async 'nlu.chitChatSetup'() {
            ...
            try {
                const data = {
                    en: JSON.parse(Assets.getText('nlu/nlu-clai-en.json')),
                };
                const projectId = await Meteor.callWithPromise('project.insert', {
                    name: 'Clai',
                    // _id: `clai-${shortid.generate()}`,
                    _id: `clai-defaultid`,
                    namespace: 'bf-clai',
                    defaultLanguage: 'en',
                    languages: [],
