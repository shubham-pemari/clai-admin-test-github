...
export const createFailingTestsGroup = (projectId) => {
    ...
    Meteor.call(
        'storyGroups.insert',
        {
            ...
            //isExpanded: false, -->removed to fix concurrency list view issue
            pinned: true,
            hideIfEmpty: true,
        },
    );
...
export const createStoriesWithTriggersGroup = (projectId) => {
    ...
    Meteor.call(
        'storyGroups.insert',
        {
            ...
            //isExpanded: true, -->removed to fix concurrency list view issue
            pinned: true,
            expandedUsers:[]
        },
    );
...
export const createUnpublishedStoriesGroup = (projectId) => {
    ...
    Meteor.call(
        'storyGroups.insert',
        {
            ...
            //isExpanded: false, -->removed to fix concurrency list view issuee
            pinned: true,
            expandedUsers:[]
        },
    );
...
Meteor.methods({
    ...
    'storyGroups.setExpansion'(storyGroup) {
        ...
        check(storyGroup, Object);
        // logic added to fix concurrency list view issue


        try {
            const {_id} = storyGroup;

            let expandedusers =StoryGroups.findOne({ _id: _id }, { fields: { expandedUsers: 1 } });

            if(expandedusers.expandedUsers!=undefined){
            ...
            if(flag==true){
                index = arr_users.indexOf(Meteor.userId())
                arr_users.splice(index, 1)
            }
            else
                {
            ...
            return StoryGroups.update({ _id }, { $set: { expandedUsers:arr_users} });
            }
            else{
                var temp_arr = []
                temp_arr.push(Meteor.userId())
                return StoryGroups.update({ _id }, { $set: { expandedUsers:temp_arr} });
            }
