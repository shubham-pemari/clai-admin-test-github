...
function ResponseMetadataForm({
        ...
        },
        {
            ...
            render: () => (
                <div className={readOnlyClass}>
                            ...
                            <ListField name='pageChangeCallbacks.pageChanges'>
                                <ListItemField name='$'>
                                    <NestField name=''>
                                        ...
                                        <BoolField name='current'/>
                                        <AutoField name='url' />
                                        <AutoField name='elementselector' />
                            ...
                            <ListField name='pageEventCallbacks.pageEvents'>
                                <ListItemField name='$'>
                                    <NestField name=''>
                                        ...
                                        <IntentField name='errorIntent' label='Error intent' />
                                    </NestField>
                                </ListItemField>
                            </ListField>
                    ...
                    <ToggleField name='delayTime.enabled' className='toggle' label='Delay Time' />
                    <DisplayIf condition={context => context.model.delayTime && context.model.delayTime.enabled}>
                        <>
                            <Message
                                info
                                content={(
                                    <>
                                        In each <strong>Page Event</strong> you can defined a <strong>delay time</strong> to be added to the element<br />
                                        The <strong>delay</strong> will be on the message appearance as the event is triggered<br />
                                    </>
                                )}
                            />
                            <AutoField name='delayTime.delay' label='Delay Time' data-cy='custom-message-css' />
                        </>
                    </DisplayIf>
                </div>
            ),
                ...
                <div className={readOnlyClass}>
                    ...
                    <DisplayIf condition={context => context.model.customCss && context.model.customCss.enabled}>
                        <>
                            ...
                            <DisplayIf condition={context => context.model.customCss && context.model.customCss.style === 'custom'}>
                                <AutoField name='customCss.buttoncss' label='Button Css' data-cy='custom-message-css' />
                            </DisplayIf>
                        </>
                    </DisplayIf>
                </div>
    ...
    const postProcess = (model) => {
        ...
        if (newModel.delayTime && !newModel.delayTime.enabled) delete newModel.delayTime;
        ...
        if (newModel.delayTime && newModel.delayTime.enabled) delete newModel.delayTime.enabled;
    ...
    const getPageChangeErrors = ({ pageChangeCallbacks }) => {
        ...
        if (pageChangeCallbacks && pageChangeCallbacks.enabled && pageChangeCallbacks.pageChanges && pageChangeCallbacks.pageChanges.length) {
            ...
            pageChangeCallbacks.pageChanges.forEach((i) => {
                if (!i.callbackIntent || !i.callbackIntent.length < 0) { missing.push(i); }
            });
            if (missing.length) {
                errors.push({ name: 'pageChangeCallback.pageChanges', message: 'One of your Page Changes listener has a URL/Element Selector or an Intent Callback missing' });
            }
    ...
    const validator = (model) => {
        ...
        } else if(model.customCss && model.customCss.enabled && !model.customCss.buttoncss){
            errors.push({ name: 'customCss', message: 'You enabled Custom CSS but you did not set the Buttoncss property' });
        } 
        
        if(model.delayTime && model.delayTime.enabled && !model.delayTime.delay){
            errors.push({ name: 'delayTime', message: 'You enabled Delay Time but you did not set the Delay property' });
        }
    ...
    const preprocessModel = (model) => {
        ...
        if (newModel.delayTime && newModel.delayTime.delay) newModel.delayTime.enabled = true;
