...
const treeReducer = (externalMutators = {}) => (tree, instruction) => {
    ...
    if (expand) {
        //-->change to fix concurrency list view issue
        const { id, type } = tree.items[expand];
        var setvalexp= false
        try{
                var data = StoryGroups.findOne({ _id: id}, { fields: { expandedUsers: 1 } });
                var arr_users = data.expandedUsers
                var flag= false
                
                for(var i=0 ; i<arr_users.length;i++){
                        if(arr_users[i]==Meteor.userId()){
                            flag=true
                        }
                }
                if (flag==true){
                ...
                else setvalexp =false
            }
    catch(e){
                console.log(e.message)
            }
        setSomethingIsMutating(true);
        mutatorMapping(type, 'expand')(convertId({ id}, type), () => setSomethingIsMutating(false));
        return mutateTree(tree, expand, { isExpanded: setvalexp });
    }
    if (collapse) {
        ...
        setSomethingIsMutating(true);
        mutatorMapping(type, 'expand')(convertId({ id }, type), () => setSomethingIsMutating(false));
        return mutateTree(tree, collapse, { isExpanded: setvalexp });
    }
    ...
    var arr_users=[]
    try{
        let expandedusers =StoryGroups.findOne({ _id: parentId }, { fields: { expandedUsers: 1 } });
        let arr_users = expandedusers.expandedUsers
    }
    catch(e){
        console.log("error")
    }
        if(arr_users!=undefined){
        arr_users.push(Meteor.userId())
        StoryGroups.update({ _id: parentId  }, { $set: { expandedUsers:arr_users} });
        }
        else{
            var temp_arr = []
            temp_arr.push(Meteor.userId())
            StoryGroups.update({ _id }, { $set: { expandedUsers:temp_arr} });
        }
...
export const useStoryGroupTree = (
    ...
    const toggleExpansion = (item ,expanded)=> {

        item && setTree({ [expanded ? 'collapse' : 'expand']: item.id });
    }
