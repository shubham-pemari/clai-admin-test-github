...
import { useUploadVid } from '../hooks/video.hooks';
...
const BotResponsesContainer = (props) => {
    ...
    const [uploadVideo] = useUploadVid(name);
    ...
    const vidRender = () => (
        <ResponseContext.Provider value={{ name, uploadVideo }}>
            {renderFormat()}
        </ResponseContext.Provider>
    )
    ...
    return (
        <>
            {uploadImage ? imgRender() : (uploadVideo ? vidRender() : null)}
        </>
    );
};
