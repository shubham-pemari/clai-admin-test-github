...
const StoryGroupTreeNode = (props) => {
    ...
    const [toggle_icon , setToggle] = useState(false)

   // -->change to fix concurrency list view issue  A.C
    var toggle_arrow = false
    const toggle_arr = () => {
        try{

        var data = StoryGroups.findOne({ _id: item.id }, { fields: { expandedUsers: 1 } });
    ...
    const icon = type === 'story-group' ? (
        <Icon
            name={`caret ${!!item && toggle_arr() ? 'down' : 'right'}`}
            {...(!somethingIsMutating
                ? {
                    onClick: () => {
                      //-->change to fix concurrency list view issue A.C
                        ...
                        setToggle(toggle_icon => !toggle_icon);

                        handleToggleExpansion(item,expanded)
                    },
                    onMouseDown: (e) => {
                        e.preventDefault();
                        e.stopPropagation();
    ...
    const handleProps = !somethingIsMutating && !disableDrag
        ? {
            ...
            onMouseDown: (e, ...args) => {
                ...
                if (item && expanded){handleCollapse(item.id);} 
