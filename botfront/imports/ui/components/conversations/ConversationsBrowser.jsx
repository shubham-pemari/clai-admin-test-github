...
// import { GET_INTENTS_IN_CONVERSATIONS, GET_CONVERSATION, GET_CONVERSATIONS } from '../conversations/queries';
...
function ConversationsBrowser(props) {
    ...
    //     variables: { projectId: projectId, conversationId: activeConversationId },
    // });
    ...
    function renderIcon(tracker) {
        ...
        if (tracker.status === 'flagged') {
            return <Icon name='flag' color='yellow' />;
        }

        return '';
    }

    function renderNegativeSentiment(t) {
        // console.log(!convLoading ? convData.conversation.tracker.sentiments : null)
        return (
            <>
                {t.tracker.sentiments ? (t.tracker.sentiments.includes("neg") ? <Icon name='flag' color='red' /> : null) : null}    
            </>
        )
    }
