...
const parseBotResponse = ({ text, data: { buttons, image, video, table, starrating, attachement } = {} } = {}) => {
    let line = `- ${(text || '').replace(/\n/g, '\n  ')}`;
    const spacer = l => (l === '- ' ? '' : '\n  ');
    if (image) line += `${spacer(line)}[ ${image} ]`;
    if (video) line += `${spacer(line)}[ ${video} ]`;
    if (table) line += `${spacer(line)}[ ${table} ]`;
    // if (starrating) line += `${spacer(line)}[ ${starrating} ]`;
    if (attachement && image !== attachement) line += `${spacer(line)}[ ${attachement} ]`;
    if (attachement && video !== attachement) line += `${spacer(line)}[ ${attachement} ]`;
    if (attachement && table !== attachement) line += `${spacer(line)}[ ${attachement} ]`;
    // if (attachement && starrating !== attachement) line += `${spacer(line)}[ ${attachement} ]`;
    if (buttons) {
