...
class ChatWidgetForm extends React.Component {
    ...
    getSnippetString() {
        ...
        const snippet = `<script>!function(){let e=document.createElement("script"),t=document.head||document.getElementsByTagName("head")[0];e.src="https://skydev.pemari.com/pmb/index.js",e.async=!0,e.onload=(()=>{window.WebChat.default({customData:${JSON.stringify(
            chatWidgetSettings.customData,
        )},socketUrl:"${
            channel.base_url
    ...
    onSave = (settings) => {
        ...
        const { initPayload, closePayload, AssistantconvFont } = newSettings;
        if (initPayload) {
        ...
        if (closePayload) {
            newSettings.closePayload = closePayload.startsWith('/')
                ? closePayload
                : `/${closePayload}`;
        }
        ...
        Meteor.call(
            'project.update',
            {
                ...
                AssistantconvFont: AssistantconvFont
            },
        ...
        return (
            <>
                ...
                >
                    ...
                    <InfoField
                        required={false}
                        info='The payload that will be sent when a user closes the chat window. The response to this payload is a close message'
                        Component={IntentField}
                        label='Close payload'
                        name='closePayload'
                    />
                    ...
                    <ColorField label='Widget Title color' name='widgetTitleColor' />
                    <ColorField label='Widget Subtitle color' name='widgetSubtitleColor' />
                    <ColorField label='Main webchat color' name='mainColor' />
                    <ColorField label='Button webchat color' name='buttonColor' />
                    <ColorField
                    ...
                    <Accordion>
                        ...
                        <Accordion.Content active={advancedVisible}>
                            ...
                            />
                            <InfoField
                                label='Disable close payload'
                                required={false}
                                info='If set, close payload will not appear'
                                Component={ToggleField}
                                name='disableClosePayload'
                            />
                            <InfoField
                            ...
                            />
                            <Divider />
                            <LongTextField label='Widget buttons style' name='btncss' data-cy='Buttons Css' />
                            <InfoField
                        name='UserconvFont'
                        label='User Conversation Font'
                        info='max'
                        data-cy='user-conversation-font'
                    />
                    <InfoField
                        name='AssistantconvFont'
                        label='Assistant Conversation Font'
                        info='max'
                        data-cy='assistant-conversation-font'
                    />
                            <InfoField
                        name='Widgetitlefont'
                        label='Widget Title Font'
                        info='max'
                        data-cy='wiget-title-font'
                    />
                    <InfoField
                        name='WidgetSubtitlefont'
                        label='Widget Subtitle Font'
                        info='Min'
                        data-cy='wiget-Subtitle-font'
                    />

                    <InfoField
                        name='tooltipFontSize'
                        label='Tooltip Text Font'
                        info='Min'
                        data-cy='tooltip-Text-font'
                    />

                    <InfoField
                        name='tooltipBorderSize'
                        label='Tooltip Border Size'
                        info='Min'
                        data-cy='tooltip-border-size'
                    />
    ...
    renderContents = () => {
        ...
        const { initPayload, closePayload } = chatWidgetSettings;
        if (initPayload) {
        ...
        if (closePayload) {
            chatWidgetSettings.closePayload = closePayload.startsWith('/')
                ? closePayload.slice(1)
                : closePayload;
        }
