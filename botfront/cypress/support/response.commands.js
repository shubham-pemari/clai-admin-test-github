...
Cypress.Commands.add('setVideo', (url, n = 0) => {
    ...
    cy.dataCy('video-container').eq(n)
        .find('vid').should('have.attr', 'src')
        .and('equal', url);
});

Cypress.Commands.add('setTable', (data, n = 0) => {
    cy.dataCy('table-container').then(els => expect(els).to.have.length.of.at.least(n + 1));
    cy.wait(100);
    cy.dataCy('table-container').eq(n)
        .findCy('set-table')
        .click();
    cy.dataCy('table-input').find('input').type(`${data}{enter}`);
    cy.dataCy('table-container').eq(n)
        .find('data').should('have.attr', 'data')
        .and('equal', data);
});
